...
class List(list):
    ...
    def append(self, obj):
        ...
        if self._length and len(self) >= self._length:
            raise Exception("max length: %s exceeded" % self._length)
...
class Index(object):
    ...
    def create(self, conn):
        ...
        conn.ensure_index(self._key, drop_dups=self._drop_dups, background=self._background,
                unique=self._unique, min=self._min, max=self._max, name=self._name)
